function getTemplate (placeholder, data) {
  const itemsList = data.selectData.map(x => `<div data-type="list-item" class="select__item" data-url="">${data.includeImg ? `<img data-type="item-img" src="${x.img}" width="15px">` : ``}${data.includeTitle ? `<span data-type="item-title" class="title">${x.title}</span>` : ``}<span data-type="item-text" class="text">${x.text}</span></div>`)
  return `
